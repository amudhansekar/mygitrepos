...
public class Display extends JFrame implements MouseListener {
	...
	private boolean dis = (int)(Math.random()*2)+1==1;
	private int[] board= new int[90];
	private int strickenx;
	private int strickeny;
	
	private int blahblah=1;
	private int rahrah=0;
	private int chase=0;
	...
	public void initComputerShips() {
		...
		int[] poss = new int[grid.getRows()*grid.getColumns()];
		
		
		String[] names = {"Patrol","Submarine","Destroyer","BattleShip","Carrier"};
		int[] leng= {2,3,3,4,5};
//		for(int i=0;i<5;i++){
//			boolean vh = false;
//			grid.addShip(leng[i], new Point(i,i), true,names[i]);
//		}
		
		int j = 0;
		
		while(grid.areAllShipsPlaced()==false){
			
			if(grid.addShip(leng[j], new Point((int)(Math.random()*9),(int)(Math.random()*9)), (int)(Math.random()*2)+1==1, names[j])){
				j++;
			}
			
			}
		
//		grid.addShip(3, new Point(0,0), true, "Submarine");
//		grid.addShip(3, new Point(0,0), true, "Destroyer");
//		grid.addShip(4, new Point(0,0), true, "Battleship");
//		grid.addShip(5, new Point(0,0), true, "Carrier");
...
//		boolean dis = (int)(Math.random()*2)+1==1;
//		int[] board= new int[50];
		int counter = 0;
		if(grid.gameOver()){
			System.out.println("GAME OVER");
		}else{
//		I wanted to make a strategy where the computer would choose the diagonal pieces but also
//		did it at a random because then it is more likely to hit I also tried to work on the part where the
//		ai looks for the near by pieces
		if(dis){
		...
		int point = board[(int)(Math.random()*50)];
		System.out.println(point);
		
		int[] k = Utilities.saturn(point);
		int[] u = new int[8];
		Point p = new Point(k[1],k[0]);
		
		if(grid.getFired(strickenx, strickeny)==3){
			chase=0;
			rahrah=0;
			blahblah=1;
			
		}else if(chase==1){
//			u=allthedir(stricken);
//			if(bounter==1){
//				p = new Point(u[0],u[1]);
//			}
//			else if(bounter ==2){
...
//			if(chase==1&&grid.getFired(p.getX(), p.getY())==2){
//				
//			}
...
//			}
	...
	public void hunt(int x, int y) {
		...
		if(blahblah==1){
			if(y!=9){
			grid.fire(new Point(x,y+1+rahrah));
			if(grid.getFired(x,y+1+rahrah)==1){
				rahrah++;
			}else if(grid.getFired(x,y+1+rahrah)==2||y==9){
				blahblah++;
				rahrah=0;
			}
			}else{
			...
			if(grid.getFired(x,y-1-rahrah)==1){
				rahrah++;
			}else if(grid.getFired(x,y-1-rahrah)==2||y==0){
				blahblah++;
				rahrah=0;
			}
			}else{
				grid.fire(new Point(x+1+rahrah,y));
				blahblah++;
				rahrah++;
			...
			if(x+rahrah==9&&y==9){
				grid.fire(new Point(x-1,y));
				blahblah++;
				rahrah=1;
			}else if(x+rahrah==9&&y==0){
				grid.fire(new Point(x-1,y));
				blahblah++;
				rahrah=1;
			}
			...
			if(x!=9){
			grid.fire(new Point(x+1+rahrah,y));
			if(grid.getFired(x+1+rahrah,y)==1){
				rahrah++;
			}else if(grid.getFired(x+1+rahrah,y)==2||x==9){
				blahblah++;
				rahrah=0;
			}
			}else{
			...
			if(grid.getFired(x-1-rahrah,y)==1){
				rahrah++;
			}else if(grid.getFired(x-1-rahrah,y)==2||x==0){
				blahblah=1;
				rahrah=0;
			}
			}else{
	...
	public int[] allthedir(Point p){
		//does top then bottom then right then left
		int[] justin = new int[8];
		justin[0]=p.getX();
		justin[1]=p.getY()+1;
		justin[2]=p.getX();
		justin[3]=p.getY()-1;
